...
import Practicas_2º_Trimestre_Coche_Corregido.Coche.Acce.Accesorio;


public class Concesionario {

	static Coche[] coches = new Coche[10];
	static Coche[] coche = new Coche[10];
	...
	public static void main(String[] args) {
		...
		*/
		...
		Coche c1 = new Coche("Toyota", "Dango", crearAccesorios(2));
		coches[0] = c1;
		
		c1.setAccesorios(crearAccesorios(1));
	...
	public static void crearCoche(String marca, String modelo, Accesorio[] accesorios) {
		...
		for (int posicion = 0; posicion < coches.length && contador != 1; posicion ++) {
			...
			contador++;	
				...
				System.out.println("Error: El concesionario esta lleno");
			contador++;	
			}
		}
	}
	...
	public static void mostrarCoches(Coche[]coches) {
		...
		for(int i = 0; i < coches.length; i++) {
			if(coches[i] != null) {
			precioFinal = coches[i].verPrecio() + precioFinal;
	...
	public static void informe(String marca) {
		...
		for(int i = 0; i < coches.length; i++) {
			if(coches[i] != null && coches[i].getMarca().compareToIgnoreCase(marca) == 0) {
				...
				if(coches[i] instanceof CocheCombustible) {
	...
	public static void ordenarPorPrecio(Coche[]coches) {
		for (int i = 0; i < coches.length - 1; i++) {
            for (int j = 0; j < coches.length - i - 1; j++) {
                if (coches[j] != null && coches[j + 1] != null && coches[j].verPrecio() > coches[j + 1].verPrecio()) {
                    Coche temp = coches[j];
                    coches[j] = coches[j + 1];
                    coches[j + 1] = temp;
                }
